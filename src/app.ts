...
export function createApp({ config, store }: AppContext): Express {
  ...
  app.delete("/api/runs/:id", (req, res) => {
    ...
    if (result === "not_found") return res.status(404).json({ error: "run not found", id: req.params.id });
    if (result === "in_progress") return res.status(409).json({ error: "run is in progress" });
